...
where :math:`k_{threshold} = filterThreshold \cdot \frac{T[K]^{3/2}}{p[Pa]} \frac{m^3}{mol\cdot s}`. ``filterThreshold``
is set by the user in the input file and its default value is :math:`5 \times 10^{8}`. The temperature- and
pressure-dependence is derived from kinetic theory or Chapman-Enskog theory, which can be used to estimate diffusion
limits in the gas phase. Intuitively, one would expect :math:`k_{threshold}` to correspond to the diffusion limit rate
constant, but this value is often too large in practice to allow for model generation in a reasonable amount of time.
Using the above default value for ``filterThreshold``, :math:`k_{threshold}` is slightly greater than
:math:`10^{8} \frac{m^3}{mol\cdot s}`. In general, it is recommended to set ``filterThreshold`` such that
:math:`k_{threshold}` is slightly greater than the maximum rate constants one expects to be present in the system of
interest. This will ensure that very fast reactions are not accidentally filtered out. Additionally, the diffusion
limit theory temperature- and pressure-dependence is retained, because this is still the intuition behind the reaction
filtering algorithm.
...
where :math:`k_{threshold} = 10^{-3} \cdot filterThreshold \cdot \frac{T[K]^{3/2}}{p[Pa]} \frac{m^6}{mol^2\cdot s}`.
...
constant for bimolecular reactions (in :math:`\frac{m^3}{mol\cdot s}`).
